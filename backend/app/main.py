...
async def lifespan(app: FastAPI):
    ...
    async with engine.begin() as conn:
        ...
        await conn.execute(text("ALTER TABLE users ADD COLUMN IF NOT EXISTS role VARCHAR(24) DEFAULT 'trader'"))
        await conn.execute(text("ALTER TABLE users ADD COLUMN IF NOT EXISTS telegram_chat_id VARCHAR(64) DEFAULT ''"))
        await conn.execute(text("ALTER TABLE users ADD COLUMN IF NOT EXISTS created_at TIMESTAMP DEFAULT now()"))
        await conn.execute(text("ALTER TABLE user_profiles ADD COLUMN IF NOT EXISTS user_id INTEGER REFERENCES users(id) ON DELETE CASCADE"))
        await conn.execute(text("ALTER TABLE user_profiles ADD COLUMN IF NOT EXISTS first_name VARCHAR(80) DEFAULT ''"))
        await conn.execute(text("ALTER TABLE user_profiles ADD COLUMN IF NOT EXISTS last_name VARCHAR(80) DEFAULT ''"))
        await conn.execute(text("ALTER TABLE user_profiles ADD COLUMN IF NOT EXISTS date_of_birth DATE"))
        await conn.execute(text("ALTER TABLE user_profiles ADD COLUMN IF NOT EXISTS watchlist_csv VARCHAR(4000) DEFAULT ''"))
        await conn.execute(text("ALTER TABLE user_profiles ADD COLUMN IF NOT EXISTS created_at TIMESTAMP DEFAULT now()"))
        await conn.execute(text("ALTER TABLE user_profiles ADD COLUMN IF NOT EXISTS updated_at TIMESTAMP DEFAULT now()"))
        await conn.execute(text("ALTER TABLE refresh_tokens ADD COLUMN IF NOT EXISTS user_id INTEGER REFERENCES users(id) ON DELETE CASCADE"))
        await conn.execute(text("ALTER TABLE refresh_tokens ADD COLUMN IF NOT EXISTS token_jti VARCHAR(64)"))
        await conn.execute(text("ALTER TABLE refresh_tokens ADD COLUMN IF NOT EXISTS revoked BOOLEAN DEFAULT FALSE"))
        await conn.execute(text("ALTER TABLE refresh_tokens ADD COLUMN IF NOT EXISTS expires_at TIMESTAMP DEFAULT now()"))
        await conn.execute(text("ALTER TABLE refresh_tokens ADD COLUMN IF NOT EXISTS created_at TIMESTAMP DEFAULT now()"))
        await conn.execute(text("CREATE UNIQUE INDEX IF NOT EXISTS ix_users_email ON users (email)"))
        await conn.execute(text("CREATE UNIQUE INDEX IF NOT EXISTS ix_user_profiles_user_id ON user_profiles (user_id)"))
        await conn.execute(text("CREATE UNIQUE INDEX IF NOT EXISTS ix_refresh_tokens_token_jti ON refresh_tokens (token_jti)"))
